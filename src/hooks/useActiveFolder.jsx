...
const useActiveFolder = (folders) => {
  ...
  function setIsActiveFolder(targetFolderId, mode = 'default') {
    ...
    const indexTargetFolder = folders.findIndex(folder => folder.folderId === targetFolderId)
    const indexActive = folders.findIndex(folder => folder.folderId === isActiveFolder)
    ...
    if (indexTargetFolder === 0 && folders.length >= 2) return setActiveFolder(folders[indexTargetFolder + 1].folderId)
    if (indexTargetFolder === indexActive && folders.length !== 1) return setActiveFolder(folders[indexTargetFolder - 1].folderId)
    if (indexTargetFolder === indexActive && lastFolder && folders.length !== 1) return setActiveFolder(folders[indexTargetFolder - 1].folderId)
  }
